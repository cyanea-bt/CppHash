...
namespace SHA2_256_NS
{
	class SHA2_256
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_256& finalize();  // after this, only `operator T()`, `reset()`, `toArray()`, `toString()`, `toVector()` are available
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_256& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_256& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_256& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			SHA2_256& addData(const T (&array)[N]);
			template <typename T>
			SHA2_256& addData(Span<T> inSpan);

			friend constexpr bool operator==(const SHA2_256 &left, const SHA2_256 &right)
			{
				for (int i = 0; i < 8; ++i)
				{
	...
	template <typename T>
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_256::operator T() const noexcept
	{
		static_assert(std::is_unsigned<T>::value, "");
...
{
	template <>
	struct hash<Chocobo1::SHA2_256>
	{
		CONSTEXPR_CPP17_CHOCOBO1_HASH size_t operator()(const Chocobo1::SHA2_256 &hash) const noexcept
		{
			return hash;
		}
	};
